...
class Product {
    ...
    async getOne(id) {
        ...
        if (!product) {
            throw new Error('Блюдо не найдено в БД')
        }
        return product
    }
    ...
    async create(data, img) {
        ...
        const {name, price, calories, categoryId = null} = data
        const product = await ProductMapping.create({name, price, calories, image, categoryId})
    ...
    async update(id, data, img) {
        ...
        await product.update({name, price, calories, categoryId, image})
        if (data.props) { // свойства товара
